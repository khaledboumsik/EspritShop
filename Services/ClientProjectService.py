...
class ClientProjectService:
    ...
    def get_jobs_by_project(self, ProjectID):
        session = self.Session()
        selected_jobs = session.query(ClientProject).filter_by(ProjectID=ProjectID).all()
        session.close()
        return  [ {
                "id": job.identifier,
            ...
            } for job in selected_jobs ]if selected_jobs else {}

    def get_jobs_by_client(self, ClientID):
        session = self.Session()
        selected_jobs = session.query(ClientProject).filter_by(identifier=ClientID).all()
        session.close()
        return  [ {
                "id": job.identifier,
                "ClientID": job.ClientID,
